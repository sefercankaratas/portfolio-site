...
function computeCashFlow(month, prevMonth) {
  let newMoney = 0;
  let marketMove = 0;
  // Cash-pool positions (money market funds where the user parks salary/cash) are not
  // investments themselves — moving money out of them into other assets is the user's own
  // capital being deployed, not "new money" from outside. Their own internal change (interest
  // earned) is treated as market move; transfers in/out of them are excluded from cash flow entirely.
  const CASH_POOL_CODES = new Set(['TKM']);
  ...
  const sumPositions = (curList, prevList, keyField, valueField) => {
    ...
    allKeys.forEach(k => {
      ...
      if (CASH_POOL_CODES.has(k)) {
        // Entire change (including qty moved out to fund other purchases) is excluded from
        // cash flow — it's the user's own capital shifting location, not new external money.
        return;
      }
      if (cur && prev) {
  ...
  // TKM (TEB Portföy Para Piyasası) is the user's cash pool — salary and spare cash sit here
  // before being deployed into other assets. Its own change is shown separately: money moving
  // between TKM and other assets is an internal transfer, not new money from outside.
  const tkmPrev = prevMonth.funds.find(f => f.code === 'TKM');
  const tkmCur = month.funds.find(f => f.code === 'TKM');
  const tkmChange = (tkmCur ? tkmCur.market_value_try : 0) - (tkmPrev ? tkmPrev.market_value_try : 0);

  const total = newMoney + marketMove + tkmChange;
  return { newMoney, marketMove, tkmChange, total };
}
        ...
        {tab === 'genel' && (
          <div className="fade-in">
            ...
            {prevMonth && (() => {
                ...
                const barTotal = Math.abs(flow.newMoney) + Math.abs(flow.marketMove) + Math.abs(flow.tkmChange) || 1;
                return (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      <div style={styles.barTrack}>
                        <div style={{ display: 'flex', height: '100%' }}>
                          ...
                          <div style={{ width: Math.abs(flow.tkmChange) / barTotal * 100 + '%', background: '#8f7de0' }} />
                        </div>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <span style={{ fontSize: 12, color: '#a9b4c0' }}>Yeni Yatırım</span>
                          </div>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
                            <span style={{ fontSize: 12, color: '#a9b4c0' }}>TKM Nakit Havuzu</span>
                            <span style={{ width: 8, height: 8, borderRadius: 999, background: '#8f7de0' }} />
                          </div>
                          <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 14, color: '#d7dee5', marginTop: 3 }}>{fmtTRY(flow.tkmChange, {sign:true})}</div>
                        </div>
                      </div>
                      <div style={{ fontSize: 11, color: '#576270', marginTop: 4 }}>
                        Adet/miktar artışı yeni yatırım, geri kalanı fiyat hareketinden sayılır. TKM (TEB Portföy Para Piyasası) maaş/nakit tuttuğunuz havuz olduğu için ayrı gösterilir — oraya para koymak veya oradan çekip başka varlığa aktarmak "yeni yatırım" değil, kendi nakdinizin yer değiştirmesidir.
                      </div>
                    </div>
                  </Panel>
                );
